...
const getPendingApprovals = asyncHandler(async (req, res) => {
  ...
  const auctionsWithRisk = await Promise.all(auctions.map(async (auction) => {
  ...
  res.json({
    ...
    data: {
      ...
      summary: {
        totalPending: total,
        highRisk: auctionsWithRisk.filter(a => a.riskAssessment.level === 'high').length,
        mediumRisk: auctionsWithRisk.filter(a => a.riskAssessment.level === 'medium').length,
        lowRisk: auctionsWithRisk.filter(a => a.riskAssessment.level === 'low').length
      }
    }
  });
});
...
const getApprovalStats = asyncHandler(async (req, res) => {
  ...
  const [approvalStats, categoryStats, moderatorStats] = await Promise.all([
    Auction.aggregate([
      ...
      {
        $group: {
          _id: { $dateToString: { format: "%Y-%m-%d", date: "$moderation.approvedAt" } },
  ...
  res.json({
    ...
    data: {
      ...
      moderatorStats,
      summary: {
        totalProcessed: approvalStats.reduce((sum, day) => sum + day.approved + day.rejected, 0),
        totalApproved: approvalStats.reduce((sum, day) => sum + day.approved, 0),
        totalRejected: approvalStats.reduce((sum, day) => sum + day.rejected, 0),
        approvalRate: approvalStats.length > 0 ? 
          (approvalStats.reduce((sum, day) => sum + day.approved, 0) / 
           approvalStats.reduce((sum, day) => sum + day.approved + day.rejected, 0) * 100).toFixed(1) : 0
      }
    }
  });
});
...
const getApprovalQueue = asyncHandler(async (req, res) => {
  ...
  .populate('seller.userId', 'anonymousId profile.reputation profile.totalAuctions')
  .sort({ createdAt: 1 });

  // Sort by priority (high risk first, then by creation time)
  const prioritizedQueue = pendingAuctions.map(auction => {
    let priority = 0;
    ...
    const ageHours = (Date.now() - auction.createdAt.getTime()) / (1000 * 60 * 60);
  ...
  res.json({
    ...
    data: {
      ...
      stats: {
        ...
        averageWaitTime: prioritizedQueue.reduce((sum, a) => sum + a.waitingTime, 0) / prioritizedQueue.length,
